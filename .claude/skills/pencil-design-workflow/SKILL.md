...
> 完整工作流（含 Phase 1 战略 + Phase 4 质量）见 **`.claude/skills/ai-frontend-design-workflow/SKILL.md`**。
> 复杂任务先 invoke **design-director** Agent（`.claude/agents/design-director.md`）。

## Prerequisites

- **ui-ux-pro-max**：`.claude/skills/ui-ux-pro-max/`（项目已内置）
...
```bash
python3 .claude/skills/ui-ux-pro-max/scripts/search.py "<product> <keywords>" --design-system -p "<Project Name>" -f markdown
```
