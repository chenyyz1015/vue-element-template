...
> **二次定制**：项目展示名、包名以 `design-system/PROJECT.md` 为准；同步 `package.json`、`VITE_APP_TITLE` 与 i18n。AI 命令中的 `-p` 使用 **displayName**，勿写死仓库初始名称。
...
**二次定制**：项目展示名以 `design-system/PROJECT.md` 为准（`displayName`）；`ui-ux-pro-max --persist -p` 与之保持一致，勿写死模板初始名称。
