...
导航入口：`default-layout` → `ThemeControls.vue`。语义色全局见 `src/styles/theme/semantic-vars.scss`。
...
```bash
python3 .cursor/skills/ui-ux-pro-max/scripts/search.py "<product> <keywords>" --design-system --persist -p "<appName>" --page "<page-name>"
```
...
**项目名**：`-p` 使用 `.env.*` 的 **`VITE_APP_NAME`**（说明见 `design-system/PROJECT.md`）；勿写死仓库初始名称。
